<template>
    <view class="canvas-page uni-padding-wrap">

        <view class="uni-pt20">
            <canvas class="canvas uni-mb30" canvas-id="canvas" id="canvas"></canvas>
        </view>

        <button class="uni-mb20" type="primary" @click="showDrawer">显示Drawer</button>
        <button plain type="warn" @click="navToDrag">拖拽效果</button>

        <uni-drawer
            :visible="visible"
            mode="left"
            @close="closeDrawer"
        >
            <scroll-view scroll-y class="uni-list-wrap">
                <uni-list>
                    <block v-for="(name, index) in names" :key="index">
                        <uni-list-item show-arrow :title="name" @click="handleCanvasButton(name)" />
                    </block>
                </uni-list>
            </scroll-view>
        </uni-drawer>

    </view>
</template>

<script>
    import { uniDrawer, uniList, uniListItem } from '@dcloudio/uni-ui'

    let context = null
    export default {
        name: 'Canvas',

        components: {
            uniDrawer,
            uniList,
            uniListItem
        },

        data() {
            return {
                visible: false,
                names: ["rotate", "scale", "reset", "translate", "save", "restore", "drawImage", "fillText", "fill",
                    "stroke", "clearRect", "beginPath", "closePath", "moveTo", "lineTo", "rect", "arc",
                    "quadraticCurveTo", "bezierCurveTo", "setFillStyle", "setStrokeStyle", "setGlobalAlpha",
                    "setShadow", "setFontSize", "setLineCap", "setLineJoin", "setLineWidth", "setMiterLimit"]
            }
        },

        onReady() {
            // 创建绘图上下文
            context = uni.createCanvasContext('canvas', this)
        },

        methods: {
            showDrawer() {
                this.visible = true
            },

            closeDrawer() {
                this.visible = false
            },

            handleCanvasButton(name) {
                this[name] && this[name]()
                this.visible = false
            },

            navToDrag() {
                uni.navigateTo({
                    url: '/pages/tabBar/canvas/drag/index'
                })
            },

            // 绘图功能
            rotate() {
                context.beginPath()
                context.rotate(10 * Math.PI / 180)
                context.rect(150, 75, 100, 50)
                context.fill()
                context.draw()
            },

            scale() {
                context.beginPath()
                context.rect(25, 25, 50, 50)
                context.stroke()

                context.scale(2, 2)

                context.beginPath()
                context.rect(25, 25, 50, 50)
                context.stroke()
                context.draw()
            },

            reset() {
                context.beginPath()
                context.setFillStyle('#F76260')
                context.setStrokeStyle('#000000')
                context.setFontSize(10)
                context.setGlobalAlpha(1)
                context.setShadow(0, 0, 0, 'rgba(0, 0, 0, 0)')
                context.setLineCap('butt')
                context.setLineJoin('miter')
                context.setLineWidth(1)
                context.setMiterLimit(10)
                context.draw()
            },

            translate() {
                context.beginPath()
                context.rect(10, 10, 100, 50)
                context.fill()

                context.translate(70, 70)

                context.beginPath()
                context.setFillStyle('#F76260')
                context.rect(10, 10, 100, 50)
                context.fill()
                context.draw()
            },

            save() {
                context.beginPath()
                context.setStrokeStyle('#00ff00')
                context.save()

                context.scale(2, 2)
                context.setStrokeStyle('#ff0000')
                context.rect(0, 0, 100, 100)
                context.stroke()
                context.restore()

                context.rect(0, 0, 50, 50)
                context.stroke()
                context.draw()
            },

            restore() {
                [3, 2, 1].forEach(function(item) {
                    context.beginPath()
                    context.save()
                    context.scale(item, item)
                    context.rect(10, 10, 100, 100)
                    context.stroke()
                    context.restore()
                })
                context.draw()
            },

            drawImage() {
                context.drawImage('../../../static/logo.png', 0, 0)
                context.draw()
            },

            fillText() {
                context.setStrokeStyle('#ff0000')
                context.beginPath()
                context.moveTo(0, 10)
                context.lineTo(300, 10)
                context.stroke()

                context.setFontSize(10)
                context.fillText('liang ying chu', 0, 30)
                context.setFontSize(20)
                context.fillText('liang ying chu', 100, 30)

                context.beginPath()
                context.moveTo(0, 30)
                context.lineTo(300, 30)
                context.stroke()
                context.draw()
            },

            fill() {
                context.beginPath()
                context.rect(20, 20, 150, 100)
                context.setStrokeStyle('#00ff00')
                context.stroke()
                context.fill()
                context.draw()
            },

            stroke() {
                context.beginPath()
                context.moveTo(20, 20)
                context.lineTo(20, 100)
                context.lineTo(70, 100)
                context.setStrokeStyle('#00ff00')
                context.stroke()
                context.draw()
            },

            clearRect() {
                context.setFillStyle('#ff0000')
                context.beginPath()
                context.rect(0, 0, 300, 150)
                context.fill()
                context.clearRect(20, 20, 100, 50)
                context.draw()
            },

            beginPath() {
                context.beginPath()
                context.setLineWidth(5)
                context.setStrokeStyle('#ff0000')
                context.moveTo(0, 70)
                context.lineTo(250, 75)
                context.stroke()

                context.beginPath()
                context.setStrokeStyle('#0000ff')
                context.moveTo(50, 0)
                context.lineTo(150, 130)
                context.stroke()
                context.draw()
            },

            closePath() {
                context.beginPath()
                context.setLineWidth(1)
                context.moveTo(20, 20)
                context.lineTo(20, 100)
                context.lineTo(70, 100)
                context.closePath()
                context.stroke()
                context.draw()
            },

            moveTo() {
                context.beginPath()
                context.moveTo(0, 0)
                context.lineTo(300, 150)
                context.stroke()
                context.draw()
            },

            lineTo() {
                context.beginPath()
                context.moveTo(20, 20)
                context.lineTo(20, 100)
                context.lineTo(70, 100)
                context.stroke()
                context.draw()
            },

            rect() {
                context.beginPath()
                context(20, 20, 150, 100)
                context.stroke()
                context.draw()
            },

            arc() {
                context.beginPath()
                context.setLineWidth(2)
                context.arc(75, 75, 50, 0, Math.PI * 2, true)
                context.moveTo(110, 75)
                context.arc(75, 75, 35, 0, Math.PI, false)
                context.moveTo(65, 65)
                context.arc(60, 65, 5, 0, Math.PI * 2, true)
                context.moveTo(95, 65)
                context.arc(90, 65, 5, 0, Math.PI * 2, true)
                context.stroke()
                context.draw()
            },

            quadraticCurveTo() {
                context.beginPath()
                context.moveTo(20, 20)
                context.quadraticCurveTo(20, 100, 200, 20)
                context.stroke()
                context.draw()
            },

            bezierCurveTo() {
                context.beginPath()
                context.moveTo(20, 20)
                context.bezierCurveTo(20, 100, 200, 100, 200, 20)
                context.stroke()
                context.draw()
            },

            setFillStyle() {
                ['#fef957', 'rgb(242,159,63)', 'rgb(242,117,63)', '#e87e51'].forEach(function(item, index) {
                    context.setFillStyle(item)
                    context.beginPath()
                    context.rect(75 * index, 0, 50, 50)
                    context.fill()
                })
                context.draw()
            },

            setStrokeStyle() {
                ['#fef957', 'rgb(242,159,63)', 'rgb(242,117,63)', '#e87e51'].forEach(function(item, index) {
                    context.setStrokeStyle(item)
                    context.beginPath()
                    context.rect(75 * index, 0, 50, 50)
                    context.stroke()
                })
                context.draw()
            },

            setGlobalAlpha() {
                context.setFillStyle('#000000')
                [1, 0.5, 0.1].forEach(function(item, index) {
                    context.setGlobalAlpha(item)
                    context.beginPath()
                    context.rect(75 * index, 0, 50, 50)
                    context.fill()
                })
                context.draw()
                context.setGlobalAlpha(1)
            },

            setShadow() {
                context.beginPath()
                context.setFillStyle('#ffcb1d')
                context.setShadow(10, 10, 10, 'rgba(0, 0, 0, 199)')
                context.rect(10, 10, 100, 100)
                context.fill()
                context.draw()
            },

            setFontSize() {
                [10, 20, 30, 40].forEach(function(item, index) {
                    context.setFontSize(item)
                    context.fillText('Liang Ying Chu', 20, 20 + 40 * index)
                })
                context.draw()
            },

            setLineCap() {
                context.setLineWidth(10)
                const arr = ['butt', 'round', 'square']
                arr.forEach(function(item, index) {
                    context.beginPath()
                    context.setLineCap(item)
                    context.moveTo(20, 20 + 20 * index)
                    context.lineTo(100, 20 + 20 * index)
                    context.stroke()
                })
                context.draw()
            },

            setLineJoin() {
                context.setLineWidth(10)
                const arr = ['bevel', 'round', 'miter']
                arr.forEach(function(item, index) {
                    context.beginPath()
                    context.setLineJoin(item)
                    context.moveTo(20 + 80 * index, 20)
                    context.lineTo(100 + 80 * index, 50)
                    context.lineTo(20 + 80 * index, 100)
                    context.stroke()
                })
                context.draw()
            },

            setLineWidth() {
                [2, 4, 6, 8, 10].forEach(function(item, index) {
                    context.beginPath()
                    context.setLineWidth(item)
                    context.moveTo(20, 20 + 20 * index)
                    context.lineTo(100, 20 + 20 * index)
                    context.stroke()
                })
                context.draw()
            },

            setMiterLimit() {
                const arr = [2, 4, 6, 8, 10]
                context.setLineWidth(4)
                arr.forEach(function(item, index) {
                    context.beginPath()
                    context.setMiterLimit(item)
                    context.moveTo(20 + 80 * index, 20)
                    context.lineTo(100 + 80 * index, 50)
                    context.lineTo(20 + 80 * index, 100)
                    context.stroke()
                })
                context.draw()
            }

        }
    }
</script>

<style rel="stylesheet/scss" lang="scss" scoped>
    .canvas-page {
        height: 100vh;
        background-color: #f7f7f7;

        .canvas {
            width: 100%;
            height: 500rpx;
            background-color: #ffffff;
        }

        .uni-list-wrap { height: 100%; }
    }
</style>